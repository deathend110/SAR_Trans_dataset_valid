...
for method_idx = 1:numel(cfg.methods)
    ...
    for pair_idx = 1:size(cfg.hl_pairs, 1)
        ...
        if endsWith(method, "_SFT")
            ...
            [state, fine_rows] = evaluate_pairs( ...
                state, fine_pairs, "fine", development_cache, S60, cfg, checkpoint_path);
            best_fine = sarvalid.select_best_candidate(fine_rows);
            if cfg.threshold.max_boundary_expansions > 0 && ...
                    touches_search_boundary(best_fine, fine_rows, method)
                expansion_pairs = enumerate_fine_pairs( ...
                    cfg, method, q_high, q_low, S60, best_fine);
                [state, ~] = evaluate_pairs( ...
                    state, expansion_pairs, "boundary_expand", ...
                    development_cache, S60, cfg, checkpoint_path);
            end
        end
        ...
        verification_signature = struct("experiment", cfg.experiment_name, ...
            "stage", "A_verification", "pair_cfg", locked_pair, ...
            "effective_grids", effective_grid_signature( ...
            cfg, method, q_high, q_low, S60), ...
            "threshold_seed", cfg.threshold_seed, ...
            "manifest", verify_manifest(:, ...
            ["SampleID", "Scene", "File", "CStart"]), ...
            "normalization", development.norm_stats, ...
            "normalization_protocol", cfg.normalization, ...
            "imaging", imaging_signature(S60));
        verification_context = struct( ...
            "path", string(fullfile(task_dir, "verification_checkpoint.mat")), ...
            "signature", verification_signature);
        verification = sarvalid.verify_pure_pair( ...
            verify_manifest, S60, locked_pair, development.norm_stats, ...
            cfg.diagnostics, fullfile(task_dir, "diagnostics"), ...
...
function yes = touches_search_boundary(best, rows, method)
tolerance = 1e-12;
yes = abs(best.STRdB-min(rows.STRdB)) < tolerance || ...
    abs(best.STRdB-max(rows.STRdB)) < tolerance || ...
    abs(best.FrOverBr-min(rows.FrOverBr)) < tolerance || ...
    abs(best.FrOverBr-max(rows.FrOverBr)) < tolerance;
if startsWith(method, "BARU_")
    yes = yes || abs(best.FaOverBa-min(rows.FaOverBa)) < tolerance || ...
        abs(best.FaOverBa-max(rows.FaOverBa)) < tolerance;
end
end
...
function signature = effective_grid_signature( ...
        cfg, method, q_high, q_low, S60)
if startsWith(method, "BARU_")
    alphas = cfg.baru_alpha(:);
else
...
for idx = 1:row_count
    pair = sarvalid.make_pair_config( ...
        cfg, method, q_high, q_low, alphas(idx), threshold);
    grid_h = sarvalid.resolve_acquisition( ...
        pair.H, [S60.nrn, S60.nan], S60);
    grid_l = sarvalid.resolve_acquisition( ...
        pair.L, [S60.nrn, S60.nan], S60);
    Alpha(idx) = alphas(idx);
    HRangeEff(idx) = grid_h.q_range_eff;
    HAzimuthEff(idx) = grid_h.q_azimuth_eff;
    HTotalEff(idx) = grid_h.q_total_eff;
    LRangeEff(idx) = grid_l.q_range_eff;
    LAzimuthEff(idx) = grid_l.q_azimuth_eff;
    LTotalEff(idx) = grid_l.q_total_eff;
end
signature = table(Alpha, HRangeEff, HAzimuthEff, HTotalEff, ...
...
HEntropy_Mean = summary.HEntropy_Mean;
LEntropy_Mean = summary.LEntropy_Mean;
HOffSupport_Mean = summary.HOffSupport_Mean;
LOffSupport_Mean = summary.LOffSupport_Mean;
HRangeLeakage_Mean = summary.HRangeLeakage_Mean;
LRangeLeakage_Mean = summary.LRangeLeakage_Mean;
HAzimuthLeakage_Mean = summary.HAzimuthLeakage_Mean;
LAzimuthLeakage_Mean = summary.LAzimuthLeakage_Mean;
row = table(Method, PairKey, QHigh, QLow, Alpha, HPSNR_Mean, ...
    HSSIM_Mean, LPSNR_Mean, LSSIM_Mean, DeltaPSNR_Mean, ...
    DeltaSSIM_Mean, HEntropy_Mean, LEntropy_Mean, ...
    HOffSupport_Mean, LOffSupport_Mean, ...
    HRangeLeakage_Mean, LRangeLeakage_Mean, ...
    HAzimuthLeakage_Mean, LAzimuthLeakage_Mean, SampleCount);
end
